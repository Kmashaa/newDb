...
BEGIN
    ...
    compare_constraints(dev_scheme_name, prod_scheme_name);
...
BEGIN
    ...
    FOR rec in (SELECT * FROM TABLES_TO_CREATE WHERE is_cycle = 1) LOOP
        DBMS_OUTPUT.PUT_LINE('Loopback links!!! ' || rec.path);
...
call compare_tables('TEST_DEVELOPMENT','PRODUCTION');
call compare_outline_constraints('TEST_DEVELOPMENT', 'PRODUCTION', 'MYTABLE');
call create_all_tables('TEST_DEVELOPMENT');
...
CREATE OR REPLACE PROCEDURE compare_constraints(dev_scheme_name IN VARCHAR2, prod_scheme_name IN VARCHAR2)
IS
    CURSOR get_table_name IS
    SELECT * FROM
        ((SELECT TABLE_NAME dev_name FROM ALL_TABLES
